...
package com.luaye.console.view {
	...
	public class Ruler extends Sprite{
		...
		public function start(console:Console):void{
			...
			_master.report("Ruler started. Click on two locations to measure.", -1);
		}
		...
		private function onMouseClick(e:MouseEvent):void{
			...
			if(_points.length==0){
				...
				//
				_master.report("Ruler results: (red) ["+p.x+","+p.y+"] to (orange) ["+p2.x+","+p2.y+"]", -2);
				_master.report("Distance: "+Utils.round(d,100) +"", -2);
				_master.report("Mid point: ["+mp.x+","+mp.y+"]", -2);
				_master.report("Width:"+w+", Height: "+h+"", -2);
				_master.report("Angle from first point (red): "+a1+"°", -2);
				_master.report("Angle from second point (orange): "+a2+"°", -2);
			}else{
				exit();
			}
		}
