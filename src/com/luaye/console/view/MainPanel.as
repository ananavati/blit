	...
	public class MainPanel extends AbstractPanel {
		...
		public function requestLogin(on:Boolean = true):void{
			...
			if(on){
				...
				master.report("//", -2);
				master.report("// Enter remoting password in CommandLine below...", -2);
		...
		public function getChannelsLink(limited:Boolean = false):String{
			...
			for(var ci:int = 0; ci<len;  ci++){
				...
				var channelTxt:String = (_viewingChannels.indexOf(channel)>=0) ? "<ch>"+channel+"</ch>" : channel;
				str += "<a href=\"event:channel_"+channel+"\">["+channelTxt+"]</a> ";
			}
			if(limited){
				str += "<ch><a href=\"event:channels\">"+(_channels.length>len?"...":"")+"^^ </a></ch>";
			}
			str += "</chs> ";
			return str;
		}
		...
		private function linkHandler(e:TextEvent):void{
			...
			if(e.text == "pause"){
				...
				if(master.tracing){
					master.report("Tracing turned [On]",-1);
				}else{
					master.report("Tracing turned [Off]",-1);
				}
