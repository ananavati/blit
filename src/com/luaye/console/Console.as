...
package com.luaye.console {
	...
	public class Console extends Sprite {
		...
		public function Console(pass:String = "", skin:ConsoleStyle = null) {
			...
			//
			report("Console v"+VERSION+(VERSION_STAGE?(" "+VERSION_STAGE):"")+", Happy coding!", -2);
		...
		public function bindKey(char:String, ctrl:Boolean, alt:Boolean, shift:Boolean, fun:Function ,args:Array = null):void{
			...
			if(!quiet){
				report((fun is Function?"Bined":"Unbined")+" key "+ char.toUpperCase() +""+ (ctrl?"+ctrl":"")+(alt?"+alt":"")+(shift?"+shift":"")+".",-1);
			}
		}
		...
		public function watch(o:Object,n:String = null):String{
			...
			if(!quiet) report("Watching "+className+" as <p5>"+ nn +"</p5>.",-1);
			return nn;
		}
		...
		private function _onEnterFrame(e:Event):void{
			...
			if(!_isPaused && _mm!=null){
				...
				if(arr.length>0){
					report("GARBAGE COLLECTED "+arr.length+" item(s): "+arr.join(", "),-2);
