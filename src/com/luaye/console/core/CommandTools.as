...
package com.luaye.console.core {
	...
	public class CommandTools {
		...
		public function inspect(obj:Object, viewAll:Boolean= true):void {
			...
			if (viewAll && obj is DisplayObjectContainer) {
				...
				for (var ci:int = 0; ci<clen; ci++) {
					...
					props.push(""+child.name+":("+ci+")"+getQualifiedClassName(child));
				}
				if(props.length){
			...
			if (viewAll && obj is DisplayObject) {
				...
				if (theParent) {
					...
					while (theParent) {
						...
						props.push(""+pr.name+":("+(theParent?theParent.getChildIndex(pr):"")+")"+getQualifiedClassName(pr));
					}
					if(props.length){
		...
		public function printHelp():void {
			...
			report("stage.width",5);
			report("stage.scaleMode = flash.display.StageScaleMode.NO_SCALE",5);
			report("stage.frameRate = 12",5);
